...
namespace CS2019SqlAdapter
{
    public partial class Form1 : Form
    {
        ...
        private bool Identity(string firstName, string lastName, string synonym)
        {
            bool isInserted = false;
            try
            {
                int rowsAffected = 0;
                StringBuilder insertQuery = new StringBuilder();
                insertQuery.Append("SET IDENTITY_INSERT Customer ON; ");
                insertQuery.Append("INSERT INTO Customer ");
                insertQuery.Append("(");
                insertQuery.Append("CustomerId, ");
                insertQuery.Append("FirstName, ");
                insertQuery.Append("LastName, ");
                insertQuery.Append("Synonym ");
                insertQuery.Append(") ");
                ...
                int CustomerId = new Random().Next(100000, int.MaxValue);
                using (SqlCommand commandSql = new SqlCommand(insertQuery.ToString()))
                {
        ...
        private bool Uniqueidentifier(string firstName, string lastName, string synonym)
        {
            bool isInserted = false;
            try
            {
                int rowsAffected = 0;
                StringBuilder insertQuery = new StringBuilder();
                // insertQuery.Append("DECLARE @NewGuid uniqueidentifier = NEWID(); ");
                insertQuery.Append("INSERT INTO Employee ");
                insertQuery.Append("(");
                insertQuery.Append("EmployeeId, ");
                insertQuery.Append("FirstName, ");
                insertQuery.Append("LastName, ");
                insertQuery.Append("Synonym ");
                insertQuery.Append(") ");
                insertQuery.Append("VALUES ");
                insertQuery.Append("(");
                // insertQuery.Append("@NewGuid, ");
                insertQuery.Append("NEWID(), ");
                insertQuery.Append("@FirstName, ");
                insertQuery.Append("@LastName, ");
                insertQuery.Append("@Synonym ");
                insertQuery.Append(");");
 ...
 || !string.IsNullOrEmpty(textBox4.Text.Trim()))
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            if (ResultDialog == DialogResult.Yes)
            {
                ...
                string lastName = textBox3.Text.Trim();
                string synonym = textBox4.Text.Trim();
                if (Identity(firstName, lastName, synonym))
                {
                    MessageBox.Show("Identity Insert success.");
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            if (ResultDialog == DialogResult.Yes)
            {
                ...
                string lastName = textBox3.Text.Trim();
                string synonym = textBox4.Text.Trim();
                if (Uniqueidentifier(firstName, lastName, synonym))
                {
                    MessageBox.Show("Identity Insert success.");
