...
class Program
{
    ...
    private void Gigya()
    {
        ...
        UserProfile profile = GetAccountInfoRegToken(regToken);
        DeleteProfile(profile.UID);
        */

        email = $"prospect{Guid.NewGuid()}@example.com";
        // var password = GenerateRandomPassword();
        var password = "_v71PH-51D";
        ...
        var regToken = Register(prospectUser);
        var profile = GetAccountInfoRegToken(regToken);
    ...
    public UserProfile GetAccountInfoByGigyaId(string gigyaId)
    {
