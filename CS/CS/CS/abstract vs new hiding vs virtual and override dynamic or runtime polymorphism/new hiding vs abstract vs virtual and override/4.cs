...
abstract class BaseClass 
{
    ...
    {       
        return "instanceMethod(int x) in abstract BaseClass: " + (s + i).ToString() + " and " + x.ToString();
    }
...
{
    public new string instanceMethod(int x)  //  Use the 'new' keyword if hiding was intended.
    {       
        return "instanceMethod(int x) in DerivedClass: " + (s + i).ToString();
    }
    ...
    public void basevirtualMethod() // #Note // *Note // @Note
    {
        Console.WriteLine(base.virtualMethod()); 
    }

    public void baseinstanceMethod(int x) // #Note // *Note // @Note
    {
        Console.WriteLine(base.instanceMethod(x)); 
    }

    /*
    public void basestaticMethod(BaseClass bcrp) // $Note  
    {
        Console.WriteLine(BaseClass.staticMethod(bcrp)); // Alternative: can be directly called in static void Main()
    }
    */
}
...
class MainClass
{
    static void Main()
    {
        ...
        Console.WriteLine(bcr.instanceMethod(1));                       // BaseClass   
        Console.WriteLine(dc.instanceMethod(1));                        // DerivedClass 
        Console.WriteLine(((BaseClass)dc).instanceMethod(1));           // BaseClass   
        Console.WriteLine();                                                                  
        ...
        dc.basevirtualMethod(); 
        Console.WriteLine();

        dc.baseinstanceMethod(1);
        Console.WriteLine();  

        // dc.basestaticMethod(bcr);

        Console.WriteLine(BaseClass.staticMethod(bcr)); // Alternative
        Console.WriteLine();     
     }
}